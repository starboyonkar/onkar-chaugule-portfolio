      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <div className="text-2xl font-bold text-white relative group">
            ...
            <span className={`${darkMode ? 'text-white' : 'text-gray-900'}`}>Nova</span>
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item, index) => (
              <a
                key={item.name}
                href={item.href}
                className={`${darkMode ? 'text-gray-300 hover:text-blue-400' : 'text-gray-700 hover:text-blue-600'} transition-all duration-300 relative group text-sm font-medium`}
            ...
            <button
              ...
              } border-2 ${darkMode ? 'border-slate-600' : 'border-gray-300'}`}
            >
          ...
          <div className="md:hidden flex items-center space-x-2">
            ...
            <button
              ...
              className={`${darkMode ? 'text-gray-300 hover:text-white' : 'text-gray-700 hover:text-gray-900'} p-2 transition-colors duration-300`}
            >
        ...
        {/* Mobile Navigation */}
        {isOpen && (
          <div className={`md:hidden ${darkMode ? 'bg-slate-800/95' : 'bg-white/95'} backdrop-blur-md rounded-lg mt-2 p-4 border ${darkMode ? 'border-slate-700/50' : 'border-gray-200/50'} animate-fade-in`}>
            {navItems.map((item, index) => (
              <a
                key={item.name}
                href={item.href}
                className={`block py-3 ${darkMode ? 'text-gray-300 hover:text-blue-400 hover:bg-slate-700/30' : 'text-gray-700 hover:text-blue-600 hover:bg-gray-100/30'} transition-colors duration-300 rounded-lg px-3`}
